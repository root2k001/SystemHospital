...
public class GestionUsuarioServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			case "cerrarSesion"	:
				...
				break;
			case "actualizarFoto":
				actualizarFoto(JsonObject, request, response);
				break;
			default:
	...
	private void actualizarFoto(JsonObject jsonObject, HttpServletRequest request, HttpServletResponse response) throws IOException {
		HttpSession session = request.getSession(false);
		JsonObject jsonResponse = new JsonObject();
		response.setContentType("application/json");
		response.setCharacterEncoding("UTF-8");
		...
		if (usuario == null) {
			jsonResponse.addProperty("status", false);
			jsonResponse.addProperty("mensaje", "Sesión no activa.");
			response.getWriter().write(jsonResponse.toString());
			return;
		}
		try {
			String fotoBase64 = jsonObject.get("foto").getAsString();
			boolean ok = dao.UsuariosDao.actualizarFoto(usuario.getId(), fotoBase64);
			if (ok) {
				usuario.setFotoPerfil(fotoBase64);
				session.setAttribute("usuarioLogeado", usuario);
				jsonResponse.addProperty("status", true);
				jsonResponse.addProperty("foto", fotoBase64);
				jsonResponse.addProperty("mensaje", "Foto actualizada correctamente.");
			} else {
				jsonResponse.addProperty("status", false);
				jsonResponse.addProperty("mensaje", "No se pudo guardar la foto.");
			}
		} catch (Exception e) {
			jsonResponse.addProperty("status", false);
			jsonResponse.addProperty("mensaje", "Error al procesar la foto.");
		}
		response.getWriter().write(jsonResponse.toString());
	}
	}
