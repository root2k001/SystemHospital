...
public class GestionPacientesServlet extends HttpServlet {
    ...
    if (sesionUsuario == null) {
		...
		if (accion == null) accion = "pacientes";

		try {
			switch(accion) {

				case "stats": {
					List<Map<String,Object>> pacs = PacienteDao.obtenerTodosLosPacientes(idUsuario);
					int totalPac = pacs.size();
					java.util.List<java.util.Map<String,Object>> citas = dao.ConsultaDao.listarPorUsuario(idUsuario);
					int totalCitas = citas.size();
					java.util.Map<String,Object> stats = new java.util.LinkedHashMap<>();
					stats.put("totalPacientes", totalPac);
					stats.put("totalCitas", totalCitas);
				...
				default:
				case "pacientes": {
					List<Map<String,Object>> listaPacientes = PacienteDao.obtenerTodosLosPacientes(idUsuario);
...
private void EliminarPac(JsonObject jsonObject, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			System.out.println("Resultado eliminar: " + pacEliminado);
			
			String mensje = pacEliminado
				? "Paciente eliminado correctamente."
				: "No se encontro el paciente DNI='" + DNIPaciente + "' para usuario=" + idUsuario;
			...
			e.printStackTrace();
			json.addProperty("estado", false);
			json.addProperty("mensaje", "Error: " + e.getMessage());
	    	response.setContentType("application/json");
	    	response.setCharacterEncoding("UTF-8");
	    	out.print(json);
            out.flush();
